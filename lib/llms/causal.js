...
export class CausalLM extends Model {
   ...
   * @param {number} [options.typical=1.0] - Typical sampling cutoff.
   * @param {number} [options.stop_token=null] - Stop token ID.
   */
  *generate(
    input,
    { max_tokens = 100, top_k = 40, top_p = 0.85, temperature = 1.0, typical = 1, stop_token = null } = {}
  ) {
    ...
    for (let i = 0; i < max_tokens; i++) {
      let res =
        typical == 1
          ? native.sample_top_k_top_p(probs, top_k, top_p, temperature)
          : native.sample_typical(probs, temperature, typical)
