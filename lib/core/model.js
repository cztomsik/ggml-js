...
export class Model extends Module {
  ...
  static loadFromFile(path) {
    ...
    const header = JSON.parse(native.safetensors_header(safetensors), (k, v) => (typeof v === 'number' ? BigInt(v) : v))

    // TODO: hparams, no_alloc
    const context = Context.init({ mem_size: BigInt(700_000_000) })
    const model = new this(context)
    ...
    // load tensor data
    native.safetensors_load(safetensors, mappings)
